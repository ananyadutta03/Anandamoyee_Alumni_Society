...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($errors)) {
        $stmt = $pdo->prepare("INSERT INTO executive_committee (name, designation, committee_type, image, bio, email, phone, linkedin, sort_order, is_active) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
        $stmt->execute([$name, $designation, $committeeType, $image, $bio ?: null, $email ?: null, $phone ?: null, $linkedin ?: null, $sortOrder, $isActive]);
                ...
                <div class="row g-3 mb-3">
                    ...
                    <div class="col-md-4">
                        ...
                        <select name="committee_type" class="form-select" required>
                            <option value="advisor" <?= ($_POST['committee_type'] ?? '') === 'advisor' ? 'selected' : '' ?>>Advisor</option>
                            <option value="executive_member" <?= ($_POST['committee_type'] ?? 'executive_member') === 'executive_member' ? 'selected' : '' ?>>Executive Member</option>
                            <option value="general_member" <?= ($_POST['committee_type'] ?? '') === 'general_member' ? 'selected' : '' ?>>General Member</option>
                            <option value="life_member" <?= ($_POST['committee_type'] ?? '') === 'life_member' ? 'selected' : '' ?>>Life Member</option>
                        </select>
                    </div>
                </div>
