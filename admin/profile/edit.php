...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (!verifyCsrf($_POST['csrf_token'] ?? '')) {
        ...
        redirect(SITE_URL . '/admin/profile/edit.php');
    }
...
if (empty($batch)) {
    ...
    if (empty($errors)) {
        $stmt = $pdo->prepare("UPDATE users SET name = ?, batch = ?, email = ?, phone = ?, address = ?, bio = ?, linkedin = ?, facebook = ?, twitter = ?, profile_image = ? WHERE id = ?");
        $stmt->execute([
            $name, $batch ?: null, $email, $phone ?: null, $address ?: null, $bio ?: null,
...
<div class="admin-wrapper">
    ...
    <div class="admin-content">
        <div class="admin-topbar">
            ...
            <div class="user-info">
                <a href="<?= SITE_URL ?>/admin/index.php" class="btn btn-sm btn-outline-secondary">
    ...
    readonly>
                    ...
                    <div class="mt-4">
                        ...
                        <a href="<?= SITE_URL ?>/user/index.php" class="btn btn-outline-secondary ms-2">Cancel</a>
                    </div>
                </form>
            </div>
        </div>
